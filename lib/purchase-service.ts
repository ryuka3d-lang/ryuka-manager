...
export function obtenerInsumosStock():
  InsumoStock[] {
  return leerLista<InsumoStock>(
    INSUMOS_KEY
  ).sort((a, b) =>
...
export async function registrarCompraFilamento(
  ...
  const peso =
    Number(
      datos.pesoPorBobinaGramos
    ) || 0;

  const precioTotal =
    Number(datos.precioTotal) || 0;
...
export function registrarCompraInsumo(
  ...
  if (existente) {
    const valorAnterior =
      existente.cantidadActual *
      existente.costoUnitarioPromedio;
...
export function eliminarCompra(
  ...
  // Las compras de filamento no se revierten
  // automáticamente para evitar borrar bobinas
  // que ya pudieron utilizarse en producción.
  guardarLista(
    COMPRAS_KEY,
    compras.filter(
      (actual) =>
