...
> {
  ...
  const [
    ...
    supabase
      ...
      .select(
        "id, production_order_id, order_item_id, quantity"
      )
      .in(
        "production_order_id",
        productionIds
    ...
    supabase
      .from("orders")
      .select(
        "id, code, customer_name_snapshot"
      )
      .in("id", orderIds),
  ...
  const productionItems =
    (productionItemRows ??
      []) as ProductionOrderItemRow[];

  const orderItemIds = [
    ...new Set(
      productionItems.map(
        (row) => row.order_item_id
      )
    ),
  ];
  ...
  return productions.map(
    (production) => {
        ...
        consumptions
          ...
          .map(
            (
              ...
              spoolCode:
                row.spool_code_snapshot,

              spoolDescription:
                row.spool_description_snapshot,
      ...
      return {
        ...
        plannedPrintMinutes:
          num(
            production.planned_print_minutes
          ),

        plannedWeightGrams:
          num(
            production.planned_weight_grams
          ),
...
export async function iniciarProduccionConBobinas(
  ...
  if (
    ...
    throw new Error(
      "La orden de producción ya fue iniciada."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Esta orden ya tiene consumos registrados."
    );
  }
